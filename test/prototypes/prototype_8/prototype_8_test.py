...
class Prototype8Test(unittest.TestCase):
    ...
    def test_source_validation_2(self):
        ...
        self.experiment.edit_property(server2.get_id(), NodeProperty(ServerProperty.PROCESSING_TIME,
                                                                     self.vectors[test]['processing_time_2'],
                                                                     'EXPRESSION',
                                                                     'GENERIC', 'Seconds'))
    ...
    def test_source_validation_3(self):
        ...
        source1_server1 = self.experiment.add_path(source.get_id(), server.get_id())
        server1_server2 = self.experiment.add_path(server.get_id(), server2.get_id())
        server2_sink1 = self.experiment.add_path(server2.get_id(), sink.get_id())
        server2_sink2 = self.experiment.add_path(server2.get_id(), sink2.get_id())
        ...
        self.experiment.edit_property(server2_sink2.get_id(),
                                      NodeProperty("Weight", self.vectors[test]['weights'][1], 'EXPRESSION', 'GENERIC',
                                                   None))
